...
static bool ecapa_lid_bench_enabled() {
    static int v = -1;
    if (v < 0) {
        const char* e = std::getenv("PARAKEET_ECAPA_LID_BENCH");
        v = (e && *e && *e != '0') ? 1 : 0;
    }
    return v != 0;
}
...
static uint32_t gguf_kv_u32(gguf_context* ctx, const char* key, uint32_t fallback) {
    const int64_t id = gguf_find_key(ctx, key);
    return id < 0 ? fallback : gguf_get_val_u32(ctx, id);
}
...
extern "C" struct ecapa_lid_context* ecapa_lid_init(const char* model_path, int n_threads) {
    ...
    gguf_init_params params = {false, &ctx->weight_ctx};
    ctx->gguf = gguf_init_from_file(model_path, params);
    if (!ctx->gguf || !ctx->weight_ctx) {
        if (ctx->gguf)
            gguf_free(ctx->gguf);
        if (ctx->weight_ctx)
    ...
    m.n_mels = gguf_kv_u32(ctx->gguf, "ecapa.n_mels", 60);
    m.n_classes = gguf_kv_u32(ctx->gguf, "ecapa.n_classes", 107);
    m.n_fft_orig = gguf_kv_u32(ctx->gguf, "ecapa.n_fft", 400);
    m.fbank_bins = gguf_kv_u32(ctx->gguf, "ecapa.fbank_bins", 201);
    m.cls_type = gguf_kv_u32(ctx->gguf, "ecapa.cls_type", 0);
    m.lin_neurons = gguf_kv_u32(ctx->gguf, "ecapa.lin_neurons", 256);
    const int tok_key = gguf_find_key(ctx->gguf, "tokenizer.ggml.tokens");
    if (tok_key >= 0) {
        int n = gguf_get_arr_n(ctx->gguf, tok_key);
        m.labels.resize(n);
        for (int i = 0; i < n; i++) {
            const char* s = gguf_get_arr_str(ctx->gguf, tok_key, i);
            if (s)
                m.labels[i] = s;
        }
    }
    const int64_t n_tensors = gguf_get_n_tensors(ctx->gguf);
    for (int64_t i = 0; i < n_tensors; ++i) {
        const char* name = gguf_get_tensor_name(ctx->gguf, i);
        ggml_tensor* tensor = ggml_get_tensor(ctx->weight_ctx, name);
        if (tensor)
            m.tensors.emplace(name, tensor);
    }
    ...
    ctx->buf = ggml_backend_cpu_buffer_from_ptr(ggml_get_mem_buffer(ctx->weight_ctx),
                                                 ggml_get_mem_size(ctx->weight_ctx));
    if (!ctx->buf) {
...
extern "C" const char* ecapa_lid_detect_among(struct ecapa_lid_context* ctx, const float* samples, int n_samples,
    ...
    const char* ref_path = std::getenv("PARAKEET_ECAPA_REF_FBANK");
    ...
    if (!asp_cpu) {
        ...
        if (std::getenv("PARAKEET_ECAPA_TIMING")) {
            double asp_ms =
